...
contract IntegrationUtils is Test {
  ...
  //////////////////////////////////////////////////////////////*/
    ...
    function _generateWithdrawalProof(
        WithdrawalProofParams memory _params
    ) internal returns (ProofLib.WithdrawProof memory _proof) {
        // Generate state merkle proof
        bytes memory _stateMerkleProof = _generateMerkleProof(
            _merkleLeaves,
            _params.existingCommitment
        );
        // Generate ASP merkle proof
        bytes memory _aspMerkleProof = _generateMerkleProof(
            _aspLeaves,
            _params.label
        );
    ...
    function _generateMerkleProofMemory(
  ...
  //////////////////////////////////////////////////////////////*/
    ...
    function _balance(
        address _account,
        IERC20 _asset
    ) internal view returns (uint256 _bal) {
        if (_asset == IERC20(Constants.NATIVE_ASSET)) {
    ...
    function _hashNullifier(
        uint256 _nullifier
    ) internal pure returns (uint256 _nullifierHash) {
        _nullifierHash = PoseidonT2.hash([_nullifier]);
    }

    function _hashPrecommitment(
        uint256 _nullifier,
        uint256 _secret
    ) internal pure returns (uint256 _precommitment) {
    ...
    function _genSecretBySeed(
        string memory _seed
    ) internal pure returns (uint256 _secret) {
        _secret =
            uint256(keccak256(bytes(_seed))) %
