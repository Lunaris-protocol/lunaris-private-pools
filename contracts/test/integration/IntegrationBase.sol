...
contract IntegrationBase is IntegrationUtils {
  ...
  //////////////////////////////////////////////////////////////*/
    ...
    // CreateX Singleton
    ICreateX internal constant _CREATEX =
        ICreateX(0xba5Ed099633D3B313e4D5F7bdc1305d3c28ba5Ed);

    // Assets
    IERC20 internal constant _DAI =
    ...
    uint256 internal constant _DEFAULT_NULLIFIER =
        uint256(keccak256("NULLIFIER")) % Constants.SNARK_SCALAR_FIELD;
    uint256 internal constant _DEFAULT_SECRET =
        uint256(keccak256("SECRET")) % Constants.SNARK_SCALAR_FIELD;
    uint256 internal constant _DEFAULT_ASP_ROOT =
        uint256(keccak256("ASP_ROOT")) % Constants.SNARK_SCALAR_FIELD;
    uint256 internal constant _DEFAULT_NEW_COMMITMENT_HASH =
        uint256(keccak256("NEW_COMMITMENT_HASH")) %
  ...
  //////////////////////////////////////////////////////////////*/
    ...
    function setUp() public virtual {
        ...
        bytes memory _intializationData = abi.encodeCall(
        ...
        address _entrypointAddr = _CREATEX.deployCreate2(
            ...
            abi.encodePacked(
                ...
                abi.encode(_impl, _intializationData)
            )
        );
  ...
  //////////////////////////////////////////////////////////////*/
    ...
    function _deposit(
        ...
        _commitment.label =
            uint256(
                keccak256(abi.encodePacked(_pool.SCOPE(), ++_currentNonce))
            ) %
            Constants.SNARK_SCALAR_FIELD;
        _commitment.value = _deductFee(_params.amount, _VETTING_FEE_BPS);
        _commitment.precommitment = _hashPrecommitment(
            _commitment.nullifier,
            _commitment.secret
        );
        _commitment.hash = _hashCommitment(
            _commitment.value,
            _commitment.label,
            _commitment.precommitment
        );
        ...
        uint256 _depositorInitialBalance = _balance(
            _params.depositor,
            _params.asset
        );
        uint256 _entrypointInitialBalance = _balance(
            address(_entrypoint),
            _params.asset
        );
        ...
        vm.prank(_params.depositor);
        if (_params.asset == IERC20(Constants.NATIVE_ASSET)) {
            _entrypoint.deposit{value: _params.amount}(
                _commitment.precommitment
            );
        } else {
            _entrypoint.deposit(
        ...
        assertEq(
            _balance(_params.depositor, _params.asset),
            _depositorInitialBalance - _params.amount,
            "User balance mismatch"
        );
        assertEq(
            _balance(address(_entrypoint), _params.asset),
            _entrypointInitialBalance + _fee,
            "Entrypoint balance mismatch"
        );
        assertEq(
            _balance(address(_pool), _params.asset),
            _poolInitialBalance + _commitment.value,
  ...
  //////////////////////////////////////////////////////////////*/
    ...
    function _selfWithdraw(
        ...
        // Define pool to deposit to
        IPrivacyPool _pool = _params.commitment.asset ==
            IERC20(Constants.NATIVE_ASSET)
            ? IPrivacyPool(address(_ethPool))
            : IPrivacyPool(address(_daiPool));
        ...
        IPrivacyPool.Withdrawal memory _withdrawal = IPrivacyPool.Withdrawal({
            processooor: _params.recipient,
            data: ""
        });

        // Withdraw
        _commitment = _withdraw(
            _params.recipient,
            _pool,
            _withdrawal,
            _params,
    ...
    function _withdraw(
        ...
        uint256 _recipientInitialBalance = _balance(
            _params.recipient,
            _params.commitment.asset
        );
        uint256 _entrypointInitialBalance = _balance(
            address(_entrypoint),
            _params.commitment.asset
        );
        uint256 _poolInitialBalance = _balance(
            address(_pool),
            _params.commitment.asset
        );

        // Compute context hash
        uint256 _context = uint256(
            keccak256(abi.encode(_withdrawal, _pool.SCOPE()))
        ) % SNARK_SCALAR_FIELD;
        ...
        if (_params.revertReason == NONE) {
            ...
            assertTrue(
                _pool.nullifierHashes(_proof.pubSignals[1]),
                "Existing nullifier hash must be spent"
            );
            ...
            uint256 _withdrawnAmountAfterFees = _direct
                ? _params.withdrawnAmount
                : _deductFee(_params.withdrawnAmount, _VETTING_FEE_BPS);
  ...
  //////////////////////////////////////////////////////////////*/
    ...
    function _ragequit(
        ...
        // Check balance changes
        assertEq(
            _balance(_depositor, _commitment.asset),
            _depositorInitialBalance + _commitment.value,
            "User balance mismatch"
        );
        assertEq(
        ...
        assertEq(
            _balance(address(_pool), _commitment.asset),
            _poolInitialBalance - _commitment.value,
            "Pool balance mismatch"
        );
    }
