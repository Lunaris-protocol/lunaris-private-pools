...
contract MainnetEnvironment {
    ...
    address public implementationV1 =
        0xdD8aA0560a08E39C0b3A84BBa356Bc025AfbD4C1;
    /// @notice Entrypoint ERC1967Proxy address
    Entrypoint public proxy =
        Entrypoint(payable(0x6818809EefCe719E480a7526D76bD3e561526b46));

    /// @notice ETH Privacy Pool address
    IPrivacyPool public ethPool =
        IPrivacyPool(0xF241d57C6DebAe225c0F2e6eA1529373C9A9C9fB);
 ...
 */
contract EntrypointUpgradeIntegration is
    Test,
    IntegrationUtils,
    MainnetEnvironment
{
    ...
    function setUp() public {
        ...
        (
            _poolAddressFromConfig,
            _minimumDepositAmountFromConfig,
            _vettingFeeBPSFromConfig,
            _maxRelayFeeBPSFromConfig
        ...
        bytes32 _implementationAddressRaw = vm.load(
            address(proxy),
    ...
    function test_StateIsKept() public {
        ...
        bytes32 _initializableStorage = vm.load(
            address(proxy),
            _INITIALIZABLE_SLOT
        );
        ...
        assertTrue(
            proxy.hasRole(_OWNER_ROLE, owner),
            "Owner address must have the owner role"
        );
        assertTrue(
            proxy.hasRole(_ASP_POSTMAN, postman),
            "Postman address must have the postman role"
        );
        ...
        assertEq(
            proxy.latestRoot(),
            _latestASPRoot,
            "Root must have not changed"
        );
        // Check the latest root index has not changed
        assertEq(
            proxy.rootByIndex(_associationSetIndex),
            _latestASPRoot,
            "Index must have not changed"
        );
    }
    ...
    function test_UpdateRoot() public {
        ...
        assertEq(
            proxy.latestRoot(),
            _newRoot,
            "ASP root must have been updated"
        );
        assertEq(
            proxy.rootByIndex(_associationSetIndex + 1),
            _newRoot,
            "ASP root index must have been updated"
        );
    }
    ...
    function test_ETHDeposit() public {
        ...
        // Deposit
        vm.prank(_user);
        proxy.deposit{value: _depositAmount}(
            uint256(keccak256("precommitment"))
        );

        // Check balances were updated correctly
        assertEq(
            _entrypointBalanceBefore + _fees,
            address(proxy).balance,
            "Entrypoint balance mismatch"
        );
        assertEq(
            _poolBalanceBefore + _value,
            address(ethPool).balance,
    ...
    function test_RegisterNewPool() public {
        ...
        PrivacyPoolComplex _raiPool = new PrivacyPoolComplex(
            address(proxy),
            address(ethPool.WITHDRAWAL_VERIFIER()),
            address(ethPool.RAGEQUIT_VERIFIER()),
            _raiToken
        );
        ...
        assertEq(
            address(_poolAddressFromConfig),
            address(_raiPool),
            "Registered pool address must match"
        );
        assertEq(
            _minimumDepositAmountFromConfig,
            0.1 ether,
            "Minimum deposit amount must match"
        );
        ...
        assertEq(
            address(_raiPool),
            address(proxy.scopeToPool(_poolScope)),
            "Registered pool scope must match"
        );
    ...
    function test_WindDownAndRemovePool() public {
        ...
        assertEq(
            address(_poolAddressFromConfig),
            address(0),
            "Registered pool address must be address zero"
        );
        assertEq(
            _minimumDepositAmountFromConfig,
            0,
            "Minimum deposit amount must be zero"
        );
    ...
    function test_WithdrawFees() public {
        ...
        assertEq(
            owner.balance,
            _ownerBalanceBefore + _entrypointBalanceBefore,
            "Owner balance mismatch"
        );
        assertEq(
            address(proxy).balance,
            0,
            "Entrypoint balance should be zero"
        );
    }
    ...
    function test_DepositAndWithdrawThroughRelayer() public {
        ...
        _precommitment = _hashPrecommitment(
            _genSecretBySeed("nullifier"),
            _genSecretBySeed("secret")
        );
        ...
        _label =
            uint256(
                keccak256(abi.encodePacked(ethPool.SCOPE(), ++_currentNonce))
            ) %
            Constants.SNARK_SCALAR_FIELD;

        // Deposit
        vm.prank(_user);
        uint256 _commitmentHash = proxy.deposit{value: _depositAmount}(
            _precommitment
        );
        ...
        _leaves[0] = _label;
        bytes memory _aspMerkleProof = _generateMerkleProofMemory(
            _leaves,
            _label
        );

        (uint256 _aspRoot, , ) = abi.decode(
            _aspMerkleProof,
        ...
        IPrivacyPool.Withdrawal memory _withdrawal = IPrivacyPool.Withdrawal({
            processooor: address(proxy),
            data: abi.encode(_recipient, _relayer, 100)
        });

        // Compute context for proof gen
        _context =
            uint256(keccak256(abi.encode(_withdrawal, ethPool.SCOPE()))) %
            Constants.SNARK_SCALAR_FIELD;
        ...
        ProofLib.WithdrawProof memory _proof = abi.decode(
            _proofData,
        ...
        uint256 _withdrawnAmount = _deductFee(
            5 ether,
            _maxRelayFeeBPSFromConfig
        );
        assertEq(
            _recipientBalanceBefore + _withdrawnAmount,
            _recipient.balance
        );
    }
    ...
    function test_DepositWithdrawAndRagequit() public {
        ...
        assertEq(
            _recipientBalanceBefore + 2 ether,
            _recipient.balance,
        ...
        _ragequitProof = _generateRagequitProof(
            _value,
            _label,
            _genSecretBySeed("nullifier_2"),
            _genSecretBySeed("secret_2")
        );
    ...
    function test_DepositAndRagequit() public {
        ...
        // Generate ragequit proof
        ProofLib.RagequitProof memory _proof = _generateRagequitProof(
            _value,
            _label,
            _nullifier,
            _secret
 ...
 */
contract EntrypointBeforeAndAfterIntegration is
    Test,
    IntegrationUtils,
    MainnetEnvironment
{
    ...
    function setUp() public {
        ...
        (, , _vettingFeeBPS, ) = proxy.assetConfig(
