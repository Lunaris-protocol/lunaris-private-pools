...
contract RegisterPool is Script {
    ...
    function setUp() public {
        ...
        try
            vm.parseAddress(vm.prompt("Enter asset address (empty for native)"))
        returns (address _assetAddress) {
        ...
        _minimumDepositAmount = vm.parseUint(
            vm.prompt("Enter minimum deposit amount padded with decimals")
        );
...
contract AssignRole is Script {
    ...
    function setUp() public {
        ...
        uint256 _roleId = vm.parseUint(
            vm.prompt("Select role [owner: 0, postman: 1]")
        );
