...
abstract contract PrivacyPool is State, IPrivacyPool {
    ...
    modifier validWithdrawal(
        ...
        if (
            _proof.stateTreeDepth() > MAX_TREE_DEPTH ||
            _proof.ASPTreeDepth() > MAX_TREE_DEPTH
        ...
        if (_proof.ASPRoot() != ENTRYPOINT.latestRoot())
